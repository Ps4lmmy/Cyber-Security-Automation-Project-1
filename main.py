...
def process_eml_file(filepath):
    ...
    for fname, payload in attachments:
        ...
        vt_verdict = verdict_from_stats(vt_result)
        
        # NEW: Get ClamAV result and combine with VirusTotal
        clamav_result = scan_file_clamav(payload, fname)
        
        # Determine final verdict (ClamAV takes priority if malicious)
        final_verdict = vt_verdict
        if clamav_result.get("status") == "MALICIOUS":
            final_verdict = "MALICIOUS"
        elif clamav_result.get("status") == "ERROR" and vt_verdict == "UNKNOWN":
        ...
        report.append({
            ...
            "verdict": final_verdict  # Use the combined verdict
        })
...
if not final_df.empty:
    ...
    print("⚠️ No suspicious elements found in the email. No report generated.")
